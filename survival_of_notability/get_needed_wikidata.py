...
def get_sitelinks_from_itwiki_title(title):
    """Get Wikidata QID and sitelinks for all languages given an itwiki title."""
    try:
...
def get_qid_with_fallback(title, fallback_langs):
    ...
    qid, page_id = query_qid_and_pageid_from_title(title, "it")
    if qid:
        return qid, "it", page_id
...
def get_wikidata_info(qid):
    ...
    try:
        ...
        label = entity.get('labels', {}).get('it', {}).get('value') or \
...
def get_wikidata(input_csv, output_path_all,output_path_nominated, fallback_langs=None):
    if fallback_langs is None:
        fallback_langs = ["en", "de", "fr", "es", "ru", "ja", "nl", "pl", "pt", "ar"]
    ...
    for title in tqdm(titles[58850:], desc="Processing titles"):
